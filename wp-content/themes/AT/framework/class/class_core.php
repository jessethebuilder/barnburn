...
class white_label_themes { 
	...
	function unregister_widgets() {	 global $wpdb;
		...
		if ( isset($GLOBALS['flag-home']) && $GLOBALS['CORE_THEME']['layout_columns']['homepage'] != 3 ) { 	
			...
			register_widget( 'core_widgets_search' );
...
if($post_id != "" && is_numeric($post_id)){ $PID = $post_id; }else{ $PID = $post->ID; }
 
 
 	if( !isset($GLOBALS['CORE_THEME']['hitcounter']) || ( isset($GLOBALS['CORE_THEME']['hitcounter']) && $GLOBALS['CORE_THEME']['hitcounter'] != 1 ) ){
	
		// CHECK FOR SLOW QUERY
		$hits = get_post_meta($PID,'hits',true);
		if($hits == "" || !is_numeric($hits)){
		
			update_post_meta($PID,'hits',1);
		
		}else{		
			 
			$newhits  = $hits+1;
			$sql = " UPDATE " . $wpdb->postmeta . " SET meta_value = '".$newhits."' WHERE meta_key = 'hits' AND post_id = ".$PID." ";
			$wpdb->query(  $sql );
		...
		if(!empty($hits)){
			...
			update_post_meta($PID,'hits', 1);
			// UPDATE LAST VISITED
			//update_post_meta($PID,'last_visitor',date('Y-m-d H:i:s'));
			// UPDATE HITS ARRAY
			update_post_meta($PID,'hits_array',$data);
		}
			...
			if(isset($attach_data['sizes']['thumbnail']['file'])){
				...
				if(in_array($file['type'],$this->allowed_image_types)){ 
					...
					if (exec('/usr/local/bin/ffmpeg -version') != 127){ 
						...
						// url detection 
						if(is_int(strpos($video, 'http://')) || is_int(strpos($video, 'https://'))) { // is url
							if(is_int(strpos($video, '/'.date('Y').'/'))) { // if sorted by dirs
								$video = $uploads['path'].'/'.basename($video);
							} else { 
								$video = $uploads['basedir'].'/'.basename($video);
							}
						} else { 
							$video = $updir['basedir'].'/'.$video;
						}
...
function USER_LOGIN($username, $pass){ global $user, $CORE, $errortext; 
	...
	$STRING .= " container-fluid";	
		...
		if(isset($GLOBALS['nosidebar-left']) && isset($GLOBALS['nosidebar-right'])){
			...
			if(isset($GLOBALS['CORE_THEME']['layout_columns']['2columns']) && $GLOBALS['CORE_THEME']['layout_columns']['2columns'] == "0"){
				$STRING = "col-md-8 col-sm-12";
			
			}elseif($GLOBALS['CORE_THEME']['layout_columns']['2columns'] == 2){
			
			$STRING = "col-md-820 col-sm-12";			
			
			}else{
				$STRING = "col-md-9 col-sm-12";
			}
		}else{
...
function _e($text,$lang ="", $textdomain="premiumpress"){
	...
	if(is_array($text)){
		...
		if(isset($text[1])){
			...
			if(!isset($GLOBALS['_LANG'][$lang][$text[0]][$text[1]])){
			//echo $text[0]	 . "-- ".$text[1];
			}
...
function Language(){ $ThisLanguage = "";
...
========================================================================== */
function PAGENAV($return="", $numposts = "", $max_page = "") { global $wpdb, $wp_query; $return=""; $pages = "";
...
function SIDEBARCONTENT($type, $num = 10, $mainargs = array()){ global $wpdb; $STRING = ""; $extra = ""; $SavedContent = "";
	...
	// RETURN
	return hook_sidebarcontent_filter( $STRING  );
...
function TOOLBOX(){ global $userdata, $post, $CORE; $STRING ="";  $listinghasexpired = false; $final_payment_due = 0;
